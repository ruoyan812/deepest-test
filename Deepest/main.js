...
function updateEnemies(gs, dt) {
  ...
  for (let i = gs.enemies.length - 1; i >= 0; i--) {
    ...
    if (Math.hypot(dx, dy) < e.r + p.radius) {
      damagePlayer(gs, ENEMY_BAT_DAMAGE);
...
async function initGame(playerName) {
  ...
  function keyDown(e) {
    ...
    // ↑ near the campfire saves (and doesn't jump); otherwise ↑/Space/W jump
...
function gameLoop(now) {
  ...
  if (gs.fade) {
    updateFade(gs, dt);
  } else if (gs.status === 'playing' && !gs.backpackOpen) {
    updatePlaying(gs, dt);
  }
...
function drawCampfire(cam) {
  const gs = gameState;
  if (!gs.campfire || gs.scene !== 'level2') return;
